...
public class Pvr_UIPointer : MonoBehaviour {
    ...
    [HideInInspector]
    public float hoverDurationTimer = 1.5f;
    [HideInInspector]
    ...
    protected bool hasCheckCircle = false;
    ...
    public SpriteRenderer CircleSprite
    {
        get
        {
            if (!hasCheckCircle && circleSprite == null && transform.childCount != 0)
            {
                hasCheckCircle = true;
                circleSprite = transform.GetChild(0).GetComponent<SpriteRenderer>();
                audioSource = this.gameObject.AddComponent<AudioSource>();
                clip1 = Resources.Load<AudioClip>("NSBD_XTTS_anniu_01");
                clip2 = Resources.Load<AudioClip>("NSBD_XTTS_anniu_02");
            }
            return circleSprite;
        }
    }
    ...
    {
        Debug.Log("ResetHoverTimer");
        hoverDurationTimer = 1.5f;
    ...
    public virtual void OnUIPointerElementEnter(UIPointerEventArgs e)
    {
        ...
        {
            Debug.Log("OnUIPointerElementEnter" + ":Current=>" + (e.currentTarget == null ? "null" : e.currentTarget.name) + ":Previous=>" + (currentTarget == null ? "null" : currentTarget.name));
            ResetHoverTimer();
            if (audioSource)
            {
