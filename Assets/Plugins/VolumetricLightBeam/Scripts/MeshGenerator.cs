...
namespace VLB
{
    public static class MeshGenerator
    {
        ...
        static bool duplicateBackFaces { get { return Config.Instance.forceSinglePass; } }
        ...
        public static Mesh GenerateConeZ_Radius(float lengthZ, float radiusStart, float radiusEnd, int numSides, int numSegments, bool cap)
        {
            ...
            // X: 0 = sides ; 1 = cap
            // Y: 0 = front face ; 1 = back face (duplicateBackFaces only)
            {
            ...
            Debug.Assert(mesh.vertexCount == GetVertexCount(numSides, numSegments, genCap));
            Debug.Assert(mesh.triangles.Length == GetIndicesCount(numSides, numSegments, genCap));
        ...
        {
            return GetVertexCount(Config.Instance.sharedMeshSides, Config.Instance.sharedMeshSegments, true);
        }

        public static int GetSharedMeshIndicesCount()
        {
            return GetIndicesCount(Config.Instance.sharedMeshSides, Config.Instance.sharedMeshSegments, true);
        }
    }
}
