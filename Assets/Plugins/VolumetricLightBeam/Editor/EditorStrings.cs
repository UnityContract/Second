...
namespace VLB
{
    public static class EditorStrings
    {
        ...
        // CONFIGS
        public static readonly GUIContent ConfigGeometryLayerID = new GUIContent("Layer", "The layer the GameObjects holding the procedural cone meshes are created on");
        ...
        public static readonly GUIContent ConfigGeometryForceSinglePass = new GUIContent(" Force Single-Pass", "Force to render using only 1 pass.\nMandatory when using Render Pipeline such as HDRP and LWRP.\nOtherwise keep it disabled since it performs better.");
        ...
        public static readonly GUIContent ConfigNoise3DData = new GUIContent("Binary file", "Binary file holding the 3D Noise texture data (a 3D array). Must be exactly Size * Size * Size bytes long.");
        public static readonly GUIContent ConfigNoise3DSize = new GUIContent("Data dimension", "Size (of one dimension) of the 3D Noise data. Must be power of 2. So if the binary file holds a 32x32x32 texture, this value must be 32.");
        public static readonly GUIContent ConfigDustParticlesPrefab = new GUIContent("Prefab", "ParticleSystem prefab instantiated for the Volumetric Dust Particles feature (Unity 5.5 or above)");
